...
def build_targets(pred_boxes, target, anchors, num_anchors, num_classes, nH, nW, noobject_scale, object_scale, sil_thresh, seen):
    ...
    for b in range(nB):
        ...
        # print (cur_ious.size())
        # print (sil_thresh)
        conf_mask.view(nB,nAnchors)[b][cur_ious>sil_thresh] = 0
    if seen < 12800:
       if anchor_step == 4:
...
class RegionLoss(nn.Module):
    ...
    def forward(self, output, target):
        ...
        #print (self.anchors,nA, self.anchor_step)
        anchor_w = torch.Tensor(self.anchors).view(nA, self.anchor_step).index_select(1, torch.LongTensor([0])).cuda()
        anchor_h = torch.Tensor(self.anchors).view(nA, self.anchor_step).index_select(1, torch.LongTensor([1])).cuda()
        anchor_w = anchor_w.repeat(nB, 1).repeat(1, 1, nH*nW).view(nB*nA*nH*nW)
        anchor_h = anchor_h.repeat(nB, 1).repeat(1, 1, nH*nW).view(nB*nA*nH*nW)
        #print (x.data.view(1,nB*nA*nH*nW).size())
        # pred_boxes[0] = x.data + grid_x
        # pred_boxes[1] = y.data + grid_y
        # pred_boxes[2] = torch.exp(w.data) * anchor_w
        # pred_boxes[3] = torch.exp(h.data) * anchor_h

        pred_boxes[0] = x.data.view(1,nB*nA*nH*nW)+ grid_x
        pred_boxes[1] = y.data.view(1,nB*nA*nH*nW) + grid_y
        pred_boxes[2] = torch.exp(w.data.view(1,nB*nA*nH*nW)) * anchor_w
        pred_boxes[3] = torch.exp(h.data.view(1,nB*nA*nH*nW)) * anchor_h
        ...
        print (tcls.size(),cls_mask.size())
        print (tcls.view(-1).size())
